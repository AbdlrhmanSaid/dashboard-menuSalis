      ...
      <div className="overflow-x-auto rounded-2xl bg-white shadow-sm border border-slate-200">
        <Table className="min-w-[650px]">
          <TableHeader className="bg-slate-50/70">
            <TableRow>
              <TableHead className="font-bold text-slate-700 text-right">اسم المستخدم</TableHead>
              <TableHead className="font-bold text-slate-700 text-right">صلاحية الحساب (الدور)</TableHead>
              {isSupervisor && (
          ...
          <TableBody>
            {filteredUsers.length === 0 ? (
              <TableRow>
                <TableCell colSpan={isSupervisor ? 3 : 2} className="py-12 text-center text-slate-400">
                  لا يوجد مستخدمون مسجلون حالياً.
                </TableCell>
              </TableRow>
            ) : (
              filteredUsers.map((item) => (
                <TableRow key={item._id} className="hover:bg-slate-50/50 transition-colors duration-150">
                  {editingUserId === item._id ? (
                        ...
                        {isSupervisor ? (
                          ...
                          >
                            ...
                            <SelectContent>
                              ...
                              <SelectItem value="admin">مشرف الفرع</SelectItem>
                              <SelectItem value="supervisor">مدير النظام</SelectItem>
                            </SelectContent>
                          </Select>
                        ) : (
                          <span className="text-slate-600 font-medium">{getRoleLabel(item.role)}</span>
                        )}
                      </TableCell>
                      {isSupervisor && (
                        <TableCell>
                          <div className="flex gap-2">
                            <Button
                              ...
                              size="icon"
                              disabled={isUpdating}
                              onClick={handleSubmit((data) => onSubmit(data, item._id))}
                    ...
                    // Read Mode
                    <>
                      <TableCell className="font-semibold text-slate-800">{item.username}</TableCell>
                      <TableCell>
                        <span
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="max-w-md rounded-2xl bg-white p-6">
          <DialogHeader className="text-right">
            <DialogTitle className="text-xl font-bold text-slate-900">إضافة مستخدم جديد</DialogTitle>
          ...
          <form onSubmit={handleSubmitCreate(onCreateSubmit)} className="space-y-4 mt-2">
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="createUsername" className="font-semibold text-slate-700">اسم المستخدم</Label>
              <Input
                id="createUsername"
              ...
              {createErrors.username && (
                <p className="text-xs text-red-500">{createErrors.username.message}</p>
              )}
            </div>

            <div className="flex flex-col gap-1.5">
              <Label htmlFor="createPassword" className="font-semibold text-slate-700">كلمة المرور</Label>
              <Input
                id="createPassword"
                type="password"
              ...
              {createErrors.password && (
                <p className="text-xs text-red-500">{createErrors.password.message}</p>
              )}
            </div>

            <div className="flex flex-col gap-1.5">
              <Label htmlFor="createRole" className="font-semibold text-slate-700">الدور والصلاحية</Label>
              <Select
        ...
        <AlertDialogContent className="rounded-2xl">
          <AlertDialogHeader className="text-right">
            ...
            <AlertDialogDescription className="text-sm text-slate-500">
              هل أنت متأكد من حذف هذا الحساب نهائياً؟ هذا الإجراء لا يمكن التراجع عنه وسيفقد المستخدم إمكانية الدخول للنظام.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="flex gap-2 justify-start mt-4">
            <AlertDialogCancel onClick={() => setDeletingUserId(null)} className="rounded-xl">إلغاء</AlertDialogCancel>
            <AlertDialogAction
              disabled={isDeleting}
