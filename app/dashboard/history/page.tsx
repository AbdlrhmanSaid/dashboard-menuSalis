...
import { History, Trash2, RefreshCw, User, Clock, ShieldAlert } from "lucide-react";
  ...
  return (
    <div className="space-y-8 p-1">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-5">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">سجل الأحداث والعمليات</h1>
          <p className="text-slate-500 mt-1">
            مراقبة وعرض جميع التغييرات التي طرأت على المنتجات والفروع داخل لوحة التحكم
          </p>
        </div>
            ...
            <>
              ...
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <Button variant="destructive" className="flex items-center gap-2 rounded-xl bg-red-600 hover:bg-red-700 text-white font-semibold h-10 shadow-sm transition-all duration-150">
                ...
                <AlertDialogContent className="rounded-2xl">
                  <AlertDialogHeader className="text-right">
                    <AlertDialogTitle className="text-lg font-bold text-slate-900">تأكيد مسح سجل الأنشطة</AlertDialogTitle>
                    <AlertDialogDescription className="text-sm text-slate-500">
                      هل أنت متأكد من مسح جميع الأحداث والعمليات المسجلة نهائياً؟ لا يمكن التراجع عن هذا الإجراء بعد إتمامه.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter className="flex gap-2 justify-start mt-4">
                    <AlertDialogCancel className="rounded-xl">إلغاء</AlertDialogCancel>
                    <AlertDialogAction onClick={handleClearHistory} className="rounded-xl bg-red-600 hover:bg-red-700 text-white font-semibold">
                      نعم، امسح السجل
                    </AlertDialogAction>
                  </AlertDialogFooter>
                </AlertDialogContent>
      ...
      <div className="grid gap-6 md:grid-cols-4">
        ...
        <div className="relative overflow-hidden rounded-2xl border border-slate-100 bg-white p-6 shadow-sm transition-all hover:shadow-md">
          <div className="flex items-center justify-between">
            <div className="space-y-2">
              <p className="text-sm font-semibold text-slate-500">إجمالي الأحداث</p>
              <h3 className="text-3xl font-extrabold text-slate-900">{history.length}</h3>
            </div>
        ...
        <div className="relative overflow-hidden rounded-2xl border border-slate-100 bg-white p-6 shadow-sm transition-all hover:shadow-md">
          <div className="flex items-center justify-between">
            <div className="space-y-2">
              <p className="text-sm font-semibold text-slate-500">المستخدم الحالي</p>
          ...
          <div className="mt-4 flex items-center gap-1.5 text-xs text-blue-600 font-medium">
            <span className="capitalize">دور المستخدم: {user?.role || "مجهول"}</span>
          </div>
        </div>
      ...
      <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between border-b border-slate-50 pb-4 mb-6">
          ...
          <Badge variant="secondary" className="rounded-lg px-2.5 py-1 bg-red-50 text-red-700 hover:bg-red-50 border-0 text-xs font-semibold">
